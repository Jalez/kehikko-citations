 ...
 *
 *   bun run register            # or: PORT=7930 bun run register
 *
 ...
 * the id from the filename, so `roadmap.citations.json` is what makes this
 * `roadmap.citations`. Two files naming the same port under different names are
 ...
 * all about a stopped one, which in practice means a pane saying "nothing is
