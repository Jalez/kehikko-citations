 ...
 *
 *   bun run register            # or: PORT=7931 bun run register
 *
 ...
 *
 * ## The plugin writes this file too, and that argument still holds
 *
 ...
 * is not, and it is worth being precise about the difference, because collapsing
 * the two loses something whichever way round you do it.
 *
 ...
 *
 * The ADDRESS is not a decision anybody made. Nobody chose 7930 — they chose to
 * be framed, and 7930 is a fact about where this process happened to bind, one
 ...
 * correctly by being run. That is the one thing the package cannot work out for
 * itself, which is why it is still spelled here.
 *
 ...
 * `roadmap.citations.json` is what makes this `roadmap.citations` — and the
 * shape of the document are all in `roadmap-module-protocol/serve` now. This
