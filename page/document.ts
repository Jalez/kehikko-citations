 ...
 *
 * There is a root element and one script. Every row, filter and count is built
 * by React from what this program's own `/api` answers, because all of it
 * depends on which document is open — and because a page redrawn after a filter
 * changes must not have this file and the client holding two versions of the
 * same sentence.
 *
 * The stylesheet is not inlined here. Tailwind is imported by `src/main.tsx`
 * and served by Vite, which costs a second round trip to the same loopback
 * process that served this document — and buys not having a place where one
 * stray backtick serves half a sheet followed by a syntax error in the middle
 * of the markup.
 *
